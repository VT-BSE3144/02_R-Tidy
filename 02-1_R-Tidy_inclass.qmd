...
`country` = The country in which TB case data was reported.
`year` = Calendar year
`cases` = Test-positive cases with culture-based and ELISA-based tests in these countries with sample dates within the year above. 
`population` = The self-reported population of each country according to their census data. 
...
Let's practice reading in the HW excel file

```{r}
library(readxl)
HW_data_file <- read_excel(path = "data/HW_Data_File.xlsx")
